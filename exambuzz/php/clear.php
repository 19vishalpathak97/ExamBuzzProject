...
?>
	...
	<script>
		    ...
		    document.cookie = "vishal = " + <?php echo $number; ?>;
				...
				//	$answer= $_COOKIE['selected'];
			     //		$qq= $_COOKIE['selectedq'];
					    ...
					    //if($startbro==1){
						   // if($answer && $qq)
						   // {
						    $update = "update $current_exam_table set $column2='NULL' where q_number=$number";
						    mysqli_query($conn,$update);
						    
// $update = "update last_time set lastseen=CURRENT_TIMESTAMP where session_id='$user'";
//						    mysqli_query($conn,$update);
						    
						    $query = "select count($column2) as cc from $current_exam_table where $column2!='NULL'";
						    $rs = mysqli_query($conn,$query);
						$rss = mysqli_fetch_array($rs);
						$_SESSION['value']=$rss['cc'];
