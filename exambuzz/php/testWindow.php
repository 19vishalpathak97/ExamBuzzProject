	...
	//	echo $testcount;
		...
		else
		{
			...
			if($status=="started")
				...
				echo "<script>alert('You have already done with this test');window.location.href='studentDashboard.php';</script>";
			/*else
				//echo 3;
				echo "<script>alert('You have already done with this test');window.location.href='studentDashboard.php';</script>";*/
		}
...
<head>
	...
	//	}
					...
					if(mysqli_num_rows(mysqli_query("SHOW TABLES LIKE '".$current_exam_table."'"))==1) 
                        $table_exists=1;
                    else
                        $table_exists=0;
                    ...
                    if(!$table_exists)
                    {
                        $create = "create table $current_exam_table(q_number int primary key)";
						mysqli_query($conn,$create);
                    
                        for($gg=1;$gg<=100;$gg++)
                        {
                            $insert_in = "insert into $current_exam_table(q_number) values($gg)";
                            mysqli_query($conn,$insert_in);
                        }
                    }
   	...
   	function progress_initial()
   	{
   		$.ajax({
				type: 'GET',
				url: "progress_initial.php", 
				data: {max1:'<?php echo $total; ?>',secname:sec_name},
				success: function(data)
				{
					$("#progress").html(data);
...
<body oncontextmenu="return false;" class="noselect" ondragstart="return false;" onload = "javascript:  set(); get_question_data(); start(); progress_initial();  " >
